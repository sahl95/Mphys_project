...
try:
    ...
    star_sys = solar_System('Exoplanets_data/K2-18/star.csv', 'Exoplanets_data/K2-18/planets.csv')

times, timestep = np.linspace(0, 5*10**(4), 1234, retstep=True)
times = np.array(times, dtype='complex128')
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/K2-18')

times = np.real(times)

n_body_data = glob.glob('Exoplanets_data/K2-18/*_nbody.csv')
# print(n_body_data)
