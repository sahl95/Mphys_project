...
try:
    ...
    star_sys = solar_System('Exoplanets_data/HD_163607/star.csv', 'Exoplanets_data/HD_163607/planets.csv')

times = np.linspace(0, 2*10**(5), 1234)+0j
    # times = np.linspace(-0, .1, 500)+0j
    # times = np.linspace(10**6, 10**10, 10000)+0j
    # times = np.logspace(6, 10, 10000)+0j
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/HD_163607')

times = np.real(times)
try:
    df = pd.read_csv('Exoplanets_data/HD_163607'+'/b_nbody.csv')
except:
...
try:
    df = pd.read_csv('Exoplanets_data/HD_163607'+'/c_nbody.csv')
except:
