...
try:
    ...
    star_sys = solar_System('Exoplanets_data/HD_169830/star.csv', 'Exoplanets_data/HD_169830/planets.csv')
...
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/HD_169830')

times = np.real(times)
try:
    df = pd.read_csv('Exoplanets_data/HD_169830'+'/b_nbody_bar.csv')
except:
    df = pd.read_csv('b_nbody_bar.csv')
...
try:
    df = pd.read_csv('Exoplanets_data/HD_169830'+'/c_nbody_bar.csv')
except:
    df = pd.read_csv('c_nbody_bar.csv')
t, e = np.array(df.Time), np.array(df.e)
# plt.figure()
