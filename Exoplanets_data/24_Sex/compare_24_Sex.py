...
try:
    ...
    star_sys = solar_System('Exoplanets_data/24_Sex/star.csv', 'Exoplanets_data/24_Sex/planets.csv')
...
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/24_Sex')

times = np.real(times)
try:
    df = pd.read_csv('Exoplanets_data/24_Sex'+'/b_nbody.csv')
except:
...
try:
    df = pd.read_csv('Exoplanets_data/24_Sex'+'/c_nbody.csv')
except:
