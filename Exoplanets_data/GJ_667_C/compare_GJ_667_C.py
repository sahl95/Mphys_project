...
try:
    ...
    star_sys = solar_System('Exoplanets_data/GJ_667_C/star.csv', 'Exoplanets_data/GJ_667_C/planets.csv')

times = np.linspace(0, 3*10**(4), 1234)+0j
    # times = np.linspace(-0, .1, 500)+0j
    # times = np.linspace(10**6, 10**10, 10000)+0j
    # times = np.logspace(6, 10, 10000)+0j
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/GJ_667_C')

times = np.real(times)
try:
    df = pd.read_csv('Exoplanets_data/GJ_667_C'+'/b_nbody.csv')
except:
    df = pd.read_csv('b_nbody.csv')

f, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(5, sharex=True, figsize=(8,7))
...
try:
    df = pd.read_csv('Exoplanets_data/GJ_667_C'+'/c_nbody.csv')
except:
...
try:
    df = pd.read_csv('Exoplanets_data/GJ_667_C'+'/e_nbody.csv')
except:
...
try:
    df = pd.read_csv('Exoplanets_data/GJ_667_C'+'/f_nbody.csv')
except:
...
try:
    df = pd.read_csv('Exoplanets_data/GJ_667_C'+'/g_nbody.csv')
except:
