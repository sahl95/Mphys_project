...
try:
    ...
    star_sys = solar_System('Exoplanets_data/Kepler-419/star.csv', 'Exoplanets_data/Kepler-419/planets.csv')

times, timestep = np.linspace(0, 10*10**(4), 1234, retstep=True)
times = np.array(times, dtype='complex128')
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/Kepler-419')

times = np.real(times)

n_body_data = glob.glob('Exoplanets_data/Kepler-419/*_nbody.csv')
# print(n_body_data)
