...
try:
    ...
    star_sys = solar_System('Exoplanets_data/GJ_832/star.csv', 'Exoplanets_data/GJ_832/planets.csv')
...
eccs = simulate(star_sys, times, plot=False, plot_orbit=False, save=False, folder_name='Exoplanets_data/GJ_832')

times = np.real(times)
try:
    df = pd.read_csv('Exoplanets_data/GJ_832'+'/b_nbody.csv')
except:
...
try:
    df = pd.read_csv('Exoplanets_data/GJ_832'+'/c_nbody.csv')
except:
